...
def install_labels(cls, quiet=True, stdout=None):
    ...
    for name, property in cls.defined_properties(aliases=False, rels=False).items():
        ...
        if property.index:
            ...
            try:
                # db.cypher_query("CREATE INDEX on :{0}({1}); ".format(             # old version
                db.cypher_query("CREATE INDEX FOR (n:{0}) ON (n.{1})".format(       # new version
            ...
            if not quiet:
                stdout.write(' + Creating unique constraint for {0} on label {1} for class {2}.{3}\n'.format(
                    name, cls.__label__, cls.__module__, cls.__name__))
            try:
                # db.cypher_query("CREATE CONSTRAINT on (n:{0}) ASSERT n.{1} IS UNIQUE".format(             # old version
                db.cypher_query("CREATE CONSTRAINT {1} FOR (n:{0}) REQUIRE n.{1} IS UNIQUE".format(         # new version
                    cls.__label__, db_property))
            except ClientError as e:
                if e.code in ('Neo.ClientError.Schema.EquivalentSchemaRuleAlreadyExists',
