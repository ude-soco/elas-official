...
class E3CourseRatingsSpider(scrapy.Spider):
    name = "e3_course_ratings"
    allowed_domains = ["www.meinprof.de", "www.meinprof.de/login"]
    start_urls = ["https://www.meinprof.de/login"]
    course_url = (
        "https://www.meinprof.de/unis/nordrhein-westfalen/uni-duisburg-essen/kurse/"
    )

    def __init__(self, email=None, password=None, *args, **kwargs):
        super(E3CourseRatingsSpider, self).__init__(*args, **kwargs)
    ...
    def parse(self, response):
        ...
        return [
            scrapy.FormRequest.from_response(
                ...
                formdata={
                    "user[email]": self.http_user,
                    "user[password]": self.http_password,
                },
    ...
    def after_login(self, response):
        ...
        if notice is None or str(notice).lower().find("login erfolgreich") == -1:
            self.error("not logged in")  # type: ignore
            return
    ...
    def parse_courselist(self, response):
        ...
        for courseElement in response.css("#coursesTable tr"):
            ...
            url = "https://www.meinprof.de" + str(
                linkElement.css("a::attr(href)").get()
            )
            ...
            if url is not None:
                ...
                request = scrapy.Request(
                    rating_page, callback=self.parse_page_with_ratings
                )
    ...
    def parse_page_with_ratings(self, response):
        ...
        semesters_rating_tables = surrounding_div.css(
            'table[class="notrhover ratings full"]'
        )
        ...
        for index, semester_rating_table in enumerate(semesters_rating_tables):
            ...
            semester_ratings = self.extract_rating_comprehension(
                semester, rating_comprehension_tables
            )

            # extract detailed rating information for each rating entry and add them to the rating object created before
            ratings.extend(
                self.extract_rating_details(detail_rating_tables, semester_ratings)
            )
