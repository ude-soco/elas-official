...
OUTPUT_DATA = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__), "..", "data", "kg_data"
    )
)
OUTOUT_DATA_TO_COURSE_RECOMMENDER = os.path.abspath(
    os.path.join(elas_backend_directory, "courserecommender",
                 "data")
)
...
class KGBuilder(object):
    ...
    def extrac_triples(self):
        ...
        with open(LECTURE_DATA, "r", encoding="utf8") as f:
            ...
            for lecture_item in course_data:
                ...
                if "sws" in lecture_item:
                    lecture_dict["sws"] = lecture_item["sws"]
    ...
    def export_data(self, data, path):
        ...
        with codecs.open(OUTOUT_DATA_TO_COURSE_RECOMMENDER + path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)
