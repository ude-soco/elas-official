...
SEMESTER_DATA = os.path.abspath(os.path.join(data_directory, "semester.json"))
                ...
                for start_item in previous_courses:
                    ...
                    for end_item in current_courses:
                        ...
                        if start_course.next.is_connected(end_course):
                            next_rel = start_course.next.relationship(end_course)
...
def add_has_next(student, course):
    ...
    try:
        ...
        if not passed_index == 0:
            ...
            for instance in student.enroll.match(
                enroll_semester=previous_semester["name"], passed=True
            ):
                ...
                if previous_course_node.next.is_connected(course_node):
                    ...
                    previous_course_node.next.connect(course_node, {"count": 1})
        ...
        if not passed_index == len(semester_data) - 1:
            ...
            for instance in student.enroll.match(
                enroll_semester=next_semester["name"], passed=True
            ):
        ...
        if not current_index == 0:
            ...
            for instance in student.enroll.match(
                ...
                if next_rel.count > 1:
                    ...
                    previous_course_node.next.disconnect(course_node)
        # check if it is not the last semester,
        # course in last semester dose not have has_next relationship
