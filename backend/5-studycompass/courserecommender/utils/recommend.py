    ...
    for item in courses:
        ...
        if node["passed_number"] > 2:
            matched_courses.append(
                {
                    "name": node["name"],
                    "passed_number": node["passed_number"],
                    "passed_percentage": percentage,
                }
            )
    popular_list = sorted(
        matched_courses, key=lambda x: x["passed_number"], reverse=True
    )[:10]
    return popular_list
...
def get_course_ratings(node):
    try:
        ...
        final_list = sorted(total_rating_list, key=lambda x: x["count"], reverse=True)[
            :3
        ]
        return final_list
    except Exception as e:
