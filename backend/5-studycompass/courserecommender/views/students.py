...
def add_student(request):
    if request.method == 'POST':
        try:
            ...
            response = {"massage": "User created successfully."}
...
def show_student_courses(request):
    try:
        ...
        response = {"data": course_list,
                    "message": "show course successfully."}
        return JsonResponse(response, status=200, safe=False)
    except Exception as e:
        response = {
            "message": "an error occurred while show courses of student",
...
def show_blacklist_courses(request):
    try:
        ...
        response = {"data": blacklist,
                    "message": "show blacklist successfully."}
        return JsonResponse(response, status=200, safe=False)
    except Exception as e:
        response = {
...
def get_student_semester(request):
    ...
    start_index = next((i for i, item in enumerate(
        semester_data) if item["name"] == start_semester), -1)
...
def change_student_setting(request):
    ...
    student.save()
    return JsonResponse({"message": "Change user setting successfully"}, safe=False, status=status.HTTP_200_OK)
